...
def ship_hit(ai_settings,screen, stats, sb, ship, aliens, bullets ):
    """Responde ao fato da espaçonave ser atingida por um alien"""
    if stats.ships_left > 0:
        #Decrementa ship_left.
        stats.ships_left -= 1

        #atualiza o quadro de pontuação
        sb.prep_ships()
...
def check_play_button(ai_settings, screen, stats,sb, play_button,ship, aliens,bullets,mouse_x, mouse_y ):
    """Começa um novo jogo quando o jogador clicar no play"""
    ...
    if button_clicked and not stats.game_active:
        ...
        #Reinicia dados estatísticos do jogo
        stats.reset_stats()
        stats.game_active = True

        #reinicia as imagens da pontuação
        sb.prep_score()
        sb.prep_high_score()
        sb.prep_level()
        sb.prep_ships()
...
def check_bullet_alien_collisions(ai_settings, screen,stats,sb,ship, aliens, bullets):
    """Responde a colisões entre alienígenas e projéteis"""
    ...
    if len(aliens) == 0:
        # Se a frota inteira for destruída, inicia uma nova fase
        bullets.empty()
...
def update_bullets(ai_settings, screen,stats, sb, ship, aliens, bullets):
    """Atualiza as posições dos projéteis e se livra dos projéteis antigos"""
...
def get_number_rows(ai_settings, ship_height, alien_height):
    """Determina o numero de alienígenas que cabem em uma coluna"""
...
def create_fleet(ai_settings, screen,ship, aliens):
    ...
    #O espaçamento de um alienígena é igual a largura de um alienígena
...
def update_aliens(ai_settings,screen, stats, sb, ship, aliens, bullets):
    ...
    #Verifica se ouve colisões entre alienígenas e a espaçonave
...
def check_high_score(stats,sb):
    """Checa se existe um novo record de pontuação"""
